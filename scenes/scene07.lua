...
function scene:new(sceneManager)
    ...
    self.colectible.colectibleNeeded = 4
    ...
    self.map = maps:new().maps.map7

    self.canMove = false
    self.died = false

    self.time = 0.2
    self.timeToRestart = 0.5
    self.timeToWin = 1.3
...
function scene:update(dt)
    ...
    if self.colectible:targetReached() then
        if self.timeToWin <= 0 then
            self.sceneManager:changeScene("scene08")
        else 
...
function scene:checkCollsion(x, y)
    ...
    if collision == gameConfig.objects.button then
        ...
        if buttonY == 8 then
            if buttonX == 11 then
                self.map[6][6] = gameConfig.objects.bomb
        ...
        if buttonY == 8 then
            if buttonX == 13 then
                self.map[5][13] = gameConfig.objects.nothing
